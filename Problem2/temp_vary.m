...
%求20℃时的UI关系曲线
d0 = [-3.551,0.6857 ,0.00195,0.004804,0.2465,0.05261];
ff = inline('(d(1)*ones(1401,1)+d(2)*X(:,4)+d(3)*X(:,4).^2).*(d(4)*ones(1401,1)+d(5)*X(:,2)+d(6)*X(:,2).^2)','d','X');
[d1,dr1,~,dCovB,dMSE,dErrorModelInfo] = nlinfit(X,X(:,1),ff,d0);
d1
dMSE
UU = (d1(1)*ones(1401,1)+d1(2)*X(:,4)+d1(3)*X(:,4).^2).*(d1(4)*ones(1401,1)+d1(5)*X(:,2)+d1(6)*X(:,2).^2);
figure(2)
plot(X(:,2).*1E3,UU,'b')
hold on
plot(X(:,2).*1E3,X(:,1),'r')
...
for T=10:10:90
    U_withtemp(:,T/10) = (d1(1)*ones(1401,1)+d1(2)*(T+273)*ones(1401,1)+d1(3)*(T+273)^2*ones(1401,1)).*(d1(4)*ones(1401,1)+d1(5)*X(:,2)+d1(6)*X(:,2).^2);
end
